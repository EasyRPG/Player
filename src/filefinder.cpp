...
namespace {
	...
	boost::optional<std::string> FindFile(FileFinder::ProjectTree const& tree,
		...
		if (dir_it == tree.directories.end()) {
			return boost::none;
		}
		...
		for (char const** c = exts; *c != NULL; ++c) {
			...
			if (name_it != dir_map.end()) {
				return MakePath(std::string(tree.project_path).append("/").append(dir_it->second),
				                name_it->second);
			}
		}
	...
	std::string const& translate_rtp(std::string const& dir, std::string const& name) {
		rtp_table_type const& table =
		    Player::engine == Player::EngineRpg2k3 ? RTP_TABLE_2003 : RTP_TABLE_2000;
		...
		if (file_it == dir_it->second.end()) {
			if (is_not_ascii_filename(lower_name)) {
				...
				for (std::map<std::string, std::string>::const_iterator it = dir_it->second.begin();
				     it != file_it; ++it) {
	...
	std::string FindFile(const std::string& dir, const std::string& name, const char* exts[]) {
		...
		for (search_path_list::const_iterator i = search_paths.begin(); i != search_paths.end();
		     ++i) {
			if (!*i) {
				continue;
			}
...
EASYRPG_SHARED_PTR<FileFinder::ProjectTree> FileFinder::CreateProjectTree(std::string const& p,
	...
	if (!(Exists(p) && IsDirectory(p))) {
		return EASYRPG_SHARED_PTR<ProjectTree>();
	}
	...
	for (string_map::const_iterator i = mem.members.begin(); i != mem.members.end(); ++i) {
		(IsDirectory(MakePath(tree->project_path, i->second)) ? tree->directories
		                                                      : tree->files)[i->first] = i->second;
	}

	for (string_map::const_iterator i = tree->directories.begin(); i != tree->directories.end();
	     ++i) {
...
std::string GetFontsPath() {
	...
	if (init) {
		...
		if (SHGetFolderPath(NULL, CSIDL_FONTS, NULL, SHGFP_TYPE_CURRENT, path) == S_OK) {
			char fpath[MAX_PATH];
#ifdef UNICODE
			WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS | WC_COMPOSITECHECK, path, MAX_PATH,
			                    fpath, MAX_PATH, NULL, NULL);
#endif
...
std::string GetFontFilename(std::string const& name) {
	std::string real_name = Registry::ReadStrValue(
	    HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Fonts",
	    name + " (TrueType)");
	if (real_name.length() > 0) {
		if (FileFinder::Exists(real_name)) return real_name;
		if (FileFinder::Exists(GetFontsPath() + real_name)) return GetFontsPath() + real_name;
	}

	real_name = Registry::ReadStrValue(HKEY_LOCAL_MACHINE,
...
std::string FileFinder::FindFont(const std::string& name) {
	static const char* FONTS_TYPES[] = {".ttf", ".ttc", ".otf", ".fon", NULL, };
	...
	if (!font_filename.empty()) {
		if (FileFinder::Exists(folder_path + font_filename)) return folder_path + font_filename;

		if (FileFinder::Exists(fonts_path + font_filename)) return fonts_path + font_filename;
	}
...
void FileFinder::InitRtpPaths() {
	std::string const version_str = Player::engine == Player::EngineRpg2k
	                                    ? "2000"
	                                    : Player::engine == Player::EngineRpg2k3 ? "2003" : "";

	assert(!version_str.empty());

	std::string const company = Player::engine == Player::EngineRpg2k ? "ASCII" : "Enterbrain";
	...
	std::string rtp_path = Registry::ReadStrValue(
	    HKEY_CURRENT_USER, "Software\\" + company + "\\RPG" + version_str, key);
	if (!rtp_path.empty()) {
		add_rtp_path(rtp_path);
	}

	rtp_path = Registry::ReadStrValue(HKEY_LOCAL_MACHINE,
	                                  "Software\\" + company + "\\RPG" + version_str, key);
	if (!rtp_path.empty()) {
		add_rtp_path(rtp_path);
	}
	...
	if (!search_paths.size()) {
		Output::Warning(
		    "RTP not found. This may create missing file errors.\n"
...
void FileFinder::Quit() { search_paths.clear(); }

FILE* FileFinder::fopenUTF8(const std::string& name_utf8, char const* mode) {
#ifdef _WIN32
	return _wfopen(Utils::ToWideString(name_utf8).c_str(), Utils::ToWideString(mode).c_str());
#else
...
std::string FileFinder::FindImage(const std::string& dir, const std::string& name) {
	static const char* IMG_TYPES[] = {".bmp", ".png", ".xyz", /*".gif", ".jpg", ".jpeg",*/ NULL};
...
bool FileFinder::IsRPG2kProject(ProjectTree const& dir) {
	string_map::const_iterator const ldb_it = dir.files.find(Utils::LowerCase(DATABASE_NAME)),
...
bool FileFinder::IsEasyRpgProject(ProjectTree const& dir) {
	string_map::const_iterator const ldb_it =
	                                     dir.files.find(Utils::LowerCase(DATABASE_NAME_EASYRPG)),
	                                 lmt_it =
	                                     dir.files.find(Utils::LowerCase(TREEMAP_NAME_EASYRPG));

	return (ldb_it != dir.files.end() && lmt_it != dir.files.end());
}

std::string FileFinder::FindMusic(const std::string& name) {
	static const char* MUSIC_TYPES[] = {".wav", ".ogg", ".mid", ".midi", ".mp3", NULL};
	return FindFile("Music", name, MUSIC_TYPES);
}

std::string FileFinder::FindSound(const std::string& name) {
	static const char* SOUND_TYPES[] = {".wav", ".ogg", ".mp3", NULL};
...
FileFinder::Directory FileFinder::GetDirectoryMembers(const std::string& path,
                                                      FileFinder::Mode const m,
...
#define wpath path
	...
	if (!dir) {
		Output::Error("Error opening dir %s: %s", path.c_str(), ::strerror(errno));
		return result;
	}
	...
	while ((ent = ::readdir(dir.get())) != NULL) {
		...
		case RECURSIVE:
			if (IsDirectory(MakePath(path, name))) {
				Directory rdir =
				    GetDirectoryMembers(MakePath(path, name), RECURSIVE, MakePath(parent, name));
