...
int Game_Player::GetX() const { return location.position_x; }

void Game_Player::SetX(int new_x) { location.position_x = new_x; }

int Game_Player::GetY() const { return location.position_y; }

void Game_Player::SetY(int new_y) { location.position_y = new_y; }

int Game_Player::GetMapId() const { return location.map_id; }

void Game_Player::SetMapId(int new_map_id) { location.map_id = new_map_id; }

int Game_Player::GetDirection() const { return location.direction; }

void Game_Player::SetDirection(int new_direction) { location.direction = new_direction; }

int Game_Player::GetPrelockDirection() const { return location.prelock_direction; }

void Game_Player::SetPrelockDirection(int new_direction) {
	location.prelock_direction = new_direction;
}

bool Game_Player::IsFacingLocked() const { return location.lock_facing; }

void Game_Player::SetFacingLocked(bool locked) { location.lock_facing = locked; }

int Game_Player::GetLayer() const { return location.layer; }

void Game_Player::SetLayer(int new_layer) { location.layer = new_layer; }

int Game_Player::GetMoveSpeed() const { return location.move_speed; }

void Game_Player::SetMoveSpeed(int speed) { location.move_speed = speed; }

int Game_Player::GetMoveFrequency() const { return location.move_frequency; }

void Game_Player::SetMoveFrequency(int frequency) { location.move_frequency = frequency; }

const RPG::MoveRoute& Game_Player::GetMoveRoute() const { return location.move_route; }

void Game_Player::SetMoveRoute(const RPG::MoveRoute& move_route) {
	location.move_route = move_route;
}

int Game_Player::GetOriginalMoveRouteIndex() const { return location.original_move_route_index; }

void Game_Player::SetOriginalMoveRouteIndex(int new_index) {
	location.original_move_route_index = new_index;
}

int Game_Player::GetMoveRouteIndex() const { return location.move_route_index; }

void Game_Player::SetMoveRouteIndex(int new_index) { location.move_route_index = new_index; }

bool Game_Player::IsMoveRouteOverwritten() const { return location.move_route_overwrite; }

void Game_Player::SetMoveRouteOverwritten(bool force) { location.move_route_overwrite = force; }

bool Game_Player::IsMoveRouteRepeated() const { return location.move_route_repeated; }

void Game_Player::SetMoveRouteRepeated(bool force) { location.move_route_repeated = force; }

const std::string& Game_Player::GetSpriteName() const { return location.sprite_name; }

void Game_Player::SetSpriteName(const std::string& sprite_name) {
	location.sprite_name = sprite_name;
}

int Game_Player::GetSpriteIndex() const { return location.sprite_id; }

void Game_Player::SetSpriteIndex(int index) { location.sprite_id = index; }
...
int Game_Player::GetFlashLevel() const { return location.flash_current_level; }

void Game_Player::SetFlashLevel(int flash_level) { location.flash_current_level = flash_level; }

int Game_Player::GetFlashTimeLeft() const { return location.flash_time_left; }

void Game_Player::SetFlashTimeLeft(int time_left) { location.flash_time_left = time_left; }
...
void Game_Player::StartTeleport() { teleporting = true; }
...
bool Game_Player::IsTeleporting() const { return teleporting; }
...
void Game_Player::UpdateScroll(int last_real_x, int last_real_y) {
	...
	if (Game_Map::IsPanLocked()) return;
	...
	if (Game_Map::GetPanX() != 0 || Game_Map::GetPanY() != 0) {
		...
		if (dx > 0) Game_Map::ScrollRight(dx);
		if (dx < 0) Game_Map::ScrollLeft(-dx);
		if (dy > 0) Game_Map::ScrollDown(dy);
		if (dy < 0) Game_Map::ScrollUp(-dy);
	} else {
...
void Game_Player::UpdateNonMoving(bool last_moving) {
	...
	if (last_moving) Game_Map::UpdateEncounterSteps();
}
...
bool Game_Player::GetOnVehicle() {
	...
	vehicle_type = type;
	if (type == Game_Vehicle::Airship) through = true;
	// TODO:
	// else
	// 	ForceMoveForward();
...
bool Game_Player::GetOffVehicle() {
	if (InAirship()) {
		if (!AirshipLandOk(GetX(), GetY())) return false;
	} else {
		int front_x = Game_Map::XwithDirection(GetX(), GetDirection());
		int front_y = Game_Map::YwithDirection(GetY(), GetDirection());
		if (!CanWalk(front_x, front_y)) return false;
	}
...
bool Game_Player::IsMovable() const {
	...
	if (IsMoveRouteOverwritten()) return false;
	if (vehicle_getting_on) return false;
	if (vehicle_getting_off) return false;
	if (Game_Message::visible) return false;
	if (InAirship() && !Game_Map::GetVehicle(Game_Vehicle::Airship)->IsMovable()) return false;
	return true;
}

bool Game_Player::InVehicle() const { return vehicle_type >= 0; }

bool Game_Player::InAirship() const { return vehicle_type == Game_Vehicle::Airship; }
