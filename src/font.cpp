...
bool operator<(ShinonomeGlyph const& lhs, uint32_t const code) { return lhs.code < code; }

// Static variables.
namespace {
	typedef std::map<std::string, EASYRPG_WEAK_PTR<boost::remove_pointer<FT_Face>::type> >
	    face_cache_type;
	...
	ShinonomeGlyph const* find_gothic_glyph(uint32_t code) {
		return find_glyph(SHINONOME_GOTHIC, sizeof(SHINONOME_GOTHIC) / sizeof(ShinonomeGlyph),
		                  code);
	}

	ShinonomeGlyph const* find_mincho_glyph(uint32_t code) {
		ShinonomeGlyph const* const mincho =
		    find_glyph(SHINONOME_MINCHO, sizeof(SHINONOME_MINCHO) / sizeof(ShinonomeGlyph), code);
...
void ShinonomeFont::Render(Bitmap& bmp, int const x, int const y, Bitmap const& sys, int color,
                           unsigned code) {
	...
	unsigned const src_x = color == ColorShadow ? 16 : color % 10 * 16 + 2,
...
void FTFont::Render(Bitmap& bmp, int x, int y, Bitmap const& /* sys */, int /* color */,
                    unsigned glyph) {
	Render(bmp, x, y, Color(), glyph);
}

void FTFont::Render(Bitmap& bmp, int const x, int const y, Color const& color,
                    unsigned const glyph) {
	if (!check_face()) {
...
FontRef Font::Default(bool const m) { return m ? mincho : gothic; }
...
bool FTFont::check_face() {
	...
	if (!face_ || face_name_ != name) {
		...
		if (it == face_cache.end() || it->second.expired()) {
			...
			if (FT_New_Face(library_.get(), face_path.c_str(), 0, &face) != FT_Err_Ok) {
				Output::Error("Couldn't initialize FreeType face: %s(%s)", name.c_str(),
				              face_path.c_str());
				return false;
			}
			...
			for (int i = 0; i < face_->num_fixed_sizes; i++) {
				...
				Output::Debug("Font Size %d: %d %d %f %f %f", i, size->width, size->height,
				              size->size / 64.0, size->x_ppem / 64.0, size->y_ppem / 64.0);
			}
	...
	face_->style_flags = (bold ? FT_STYLE_FLAG_BOLD : 0) | (italic ? FT_STYLE_FLAG_ITALIC : 0);
