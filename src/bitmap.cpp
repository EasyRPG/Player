...
Bitmap::~Bitmap() { pixman_image_unref(bitmap); }
...
bool Bitmap::WritePNG(std::ostream& os) const {
	...
	EASYRPG_SHARED_PTR<pixman_image_t> dst(
	    ...
	    pixman_image_unref);
	pixman_image_composite32(PIXMAN_OP_SRC, bitmap, NULL, dst.get(), 0, 0, 0, 0, 0, 0, width,
	                         height);

	for (size_t i = 0; i < width * height; ++i) {
		uint32_t const p = data[i];
		uint8_t* out = reinterpret_cast<uint8_t*>(&data[i]);
		uint8_t a = (p >> 24) & 0xff, r = (p >> 16) & 0xff, g = (p >> 8) & 0xff,
		        b = (p >> 0) & 0xff;
		if (a != 0) {
			r = (r * 255) / a;
			g = (g * 255) / a;
	...
	png_infop info = NULL;
	BOOST_SCOPE_EXIT((&write)(&info)) { png_destroy_write_struct(&write, &info); }
	BOOST_SCOPE_EXIT_END do {}
	while (0)
		;
	...
	png_set_IHDR(write, info, width, height, 8, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE,
...
void Bitmap::AttachBitmapScreen(BitmapScreen* bitmap) { attached_screen_bitmaps.push_back(bitmap); }

void Bitmap::DetachBitmapScreen(BitmapScreen* bitmap) { attached_screen_bitmaps.remove(bitmap); }

bool Bitmap::IsAttachedToBitmapScreen() { return !attached_screen_bitmaps.empty(); }

int Bitmap::GetWidth() const { return width(); }

int Bitmap::GetHeight() const { return height(); }

Rect Bitmap::GetRect() const { return Rect(0, 0, width(), height()); }

bool Bitmap::GetTransparent() const { return format.alpha_type != PF::NoAlpha; }

Color Bitmap::GetTransparentColor() const { return GetColor(colorkey()); }
...
Bitmap::TileOpacity Bitmap::CheckOpacity(const Rect& rect) {
	...
	return all ? Bitmap::Opaque : any ? Bitmap::Partial : Bitmap::Transparent;
}
...
uint8_t Bitmap::bytes() const { return format.bytes; }

uint8_t* Bitmap::pointer(int x, int y) { return (uint8_t*)pixels() + y * pitch() + x * bytes(); }
...
void Bitmap::TransformBlit(Rect const& dst_rect_, Bitmap const& src, Rect const& src_rect,
                           double angle, double scale_x, double scale_y, int src_pos_x,
                           int src_pos_y, int dst_pos_x, int dst_pos_y, int opacity) {
	Matrix fwd = Matrix::Setup(angle, scale_x, scale_y, src_pos_x, src_pos_y, dst_pos_x, dst_pos_y);
...
void Bitmap::HSLBlit(int x, int y, Bitmap const& src, Rect const& src_rect_, double h, double s,
                     double l, double lo) {
	Rect dst_rect(x, y, 0, 0), src_rect = src_rect_;

	if (!Rect::AdjustRectangles(src_rect, dst_rect, src.GetRect())) return;
	if (!Rect::AdjustRectangles(dst_rect, src_rect, GetRect())) return;
...
void Bitmap::End() { RefreshCallback(); }

void Bitmap::End(Bitmap const&) { RefreshCallback(); }
...
FontRef const& Bitmap::GetFont() const { return font; }

void Bitmap::SetFont(FontRef const& new_font) { font = new_font; }

void Bitmap::TextDraw(int x, int y, int width, int /* height */, int color, std::string const& text,
...
void Bitmap::TextDraw(int x, int y, int width, int /* height */, Color color,
                      std::string const& text, Text::Alignment align) {
	Rect rect = GetFont()->GetSize(text);
	int dx = rect.width - width;
...
void Bitmap::TextDraw(Rect const& rect, Color color, std::string const& text,
                      Text::Alignment align) {
	TextDraw(rect.x, rect.y, rect.width, rect.height, color, text, align);
}
...
Matrix Matrix::Setup(double angle, double scale_x, double scale_y, int src_pos_x, int src_pos_y,
...
pixman_format_code_t Bitmap::find_format(const DynamicFormat& format) {
	...
	if (pcode == 0) {
		...
		Output::Error("%s\nDynamicFormat(%d, %d, %d, %d, %d, %d, %d, %d, %d, %s)",
		              "Couldn't find Pixman format for", format.bits, format.r.bits, format.r.shift,
		              format.g.bits, format.g.shift, format.b.bits, format.b.shift, format.a.bits,
		              format.a.shift, format.alpha_type == PF::Alpha ? "PF::Alpha" : "PF::NoAlpha");
	}
...
DynamicFormat Bitmap::ChooseFormat(const DynamicFormat& format) {
	...
	amask = (format.a.mask == 0)
	            ...
	            : format.a.mask;
	if (amask != 0)
		return DynamicFormat(format.bits, format.r.mask, format.g.mask, format.b.mask, amask,
		                     PF::Alpha);
	switch (format.bits) {
	case 16:
...
static void destroy_func(pixman_image_t* /* image */, void* data) { free(data); }
...
static void initialize_palette() {
	...
	palette.color = false;
	palette.rgba[0] = 0U;
	for (int i = 1; i < PIXMAN_MAX_INDEXED; i++) palette.rgba[i] = ~0U;
	palette_initialized = true;
}

void Bitmap::Init(int width, int height, void* data, int pitch, bool destroy) {
	if (!pitch) pitch = width * bytes();
	...
	if (data != NULL && destroy) pixman_image_set_destroy_function(bitmap, destroy_func, data);
}
...
void* Bitmap::pixels() { return (void*)pixman_image_get_data(bitmap); }
void const* Bitmap::pixels() const { return (void const*)pixman_image_get_data(bitmap); }

uint8_t Bitmap::bpp() const { return (pixman_image_get_depth(bitmap) + 7) / 8; }

int Bitmap::width() const { return pixman_image_get_width(bitmap); }

int Bitmap::height() const { return pixman_image_get_height(bitmap); }

uint16_t Bitmap::pitch() const { return pixman_image_get_stride(bitmap); }

uint32_t Bitmap::rmask() const { return pixel_format.r.mask; }

uint32_t Bitmap::gmask() const { return pixel_format.g.mask; }

uint32_t Bitmap::bmask() const { return pixel_format.b.mask; }

uint32_t Bitmap::amask() const { return pixel_format.a.mask; }

uint32_t Bitmap::colorkey() const { return 0; }
...
BitmapRef Bitmap::Resample(int scale_w, int scale_h, const Rect& src_rect) const {
	...
	pixman_transform_init_scale(&xform, pixman_double_to_fixed(zoom_x),
	                            pixman_double_to_fixed(zoom_y));

	pixman_image_set_transform(bitmap, &xform);

	pixman_image_composite32(PIXMAN_OP_SRC, bitmap, (pixman_image_t*)NULL, dst->bitmap,
	                         src_rect.x / zoom_x, src_rect.y / zoom_y, 0, 0, 0, 0, scale_w,
	                         scale_h);
...
void Bitmap::Blit(int x, int y, Bitmap const& src, Rect const& src_rect, int opacity) {
	...
	pixman_image_composite32(PIXMAN_OP_OVER, src.bitmap, mask, bitmap, src_rect.x, src_rect.y, 0, 0,
	                         x, y, src_rect.width, src_rect.height);

	if (mask != NULL) pixman_image_unref(mask);
...
void Bitmap::TiledBlit(int ox, int oy, Rect const& src_rect, Bitmap const& src,
                       Rect const& dst_rect, int opacity) {
	if (opacity < 0) return;
	...
	pixman_transform_t xform;
	pixman_transform_init_translate(&xform, pixman_int_to_fixed(ox), pixman_int_to_fixed(oy));

	pixman_image_set_transform(src_bm, &xform);

	pixman_image_composite32(PIXMAN_OP_OVER, src_bm, mask, bitmap, 0, 0, 0, 0, dst_rect.x,
...
void Bitmap::StretchBlit(Rect const& dst_rect, Bitmap const& src, Rect const& src_rect,
                         int opacity) {
	if (opacity < 0) return;
	...
	pixman_image_composite32(PIXMAN_OP_OVER, src.bitmap, mask, bitmap, src_rect.x / zoom_x,
	                         src_rect.y / zoom_y, 0, 0, dst_rect.x, dst_rect.y, dst_rect.width,
	                         dst_rect.height);
...
void Bitmap::TransformBlit(Rect const& dst_rect, Bitmap const& src, Rect const& /* src_rect */,
                           const Matrix& inv, int /* opacity */) {
	pixman_transform_t xform = {
	    {{pixman_double_to_fixed(inv.xx), pixman_double_to_fixed(inv.xy),
	      pixman_double_to_fixed(inv.x0)},
	     {pixman_double_to_fixed(inv.yx), pixman_double_to_fixed(inv.yy),
	      pixman_double_to_fixed(inv.y0)},
	     {pixman_double_to_fixed(0.0), pixman_double_to_fixed(0.0), pixman_double_to_fixed(1.0)}}};

	pixman_image_set_transform(src.bitmap, &xform);

	pixman_image_composite32(PIXMAN_OP_OVER, src.bitmap, (pixman_image_t*)NULL, bitmap, dst_rect.x,
	                         dst_rect.y, 0, 0, dst_rect.x, dst_rect.y, dst_rect.width,
	                         dst_rect.height);
...
void Bitmap::MaskBlit(int x, int y, Bitmap const& src, Rect const& src_rect) {
	pixman_image_composite32(PIXMAN_OP_DISJOINT_IN_REVERSE, src.bitmap, (pixman_image_t*)NULL,
	                         bitmap, src_rect.x, src_rect.y, 0, 0, x, y, src_rect.width,
	                         src_rect.height);

	RefreshCallback();
}

void Bitmap::WaverBlit(int x, int y, Bitmap const& src, Rect const& src_rect, int depth,
                       double phase, int opacity) {
	if (opacity < 0) return;
	...
	for (int i = 0; i < src_rect.height; i++) {
		...
		pixman_image_composite32(PIXMAN_OP_OVER, src.bitmap, mask, bitmap, src_rect.x,
		                         src_rect.y + i, 0, 0, x + offset, y + i, src_rect.width, 1);
	}
...
void Bitmap::FillRect(Rect const& dst_rect, const Color& color) {
	...
	pixman_rectangle16_t rect = {
	    static_cast<int16_t>(dst_rect.x),      static_cast<int16_t>(dst_rect.y),
	    static_cast<uint16_t>(dst_rect.width), static_cast<uint16_t>(dst_rect.height), };
...
void Bitmap::OpacityBlit(int x, int y, Bitmap const& src, Rect const& src_rect, int opacity) {
	if (opacity == 255) {
		if (&src != this) Blit(x, y, src, src_rect, opacity);
		return;
	}
	...
	if (&src == this) {
		...
		pixman_rectangle16_t rect = {
		    static_cast<int16_t>(src_rect.x),      static_cast<int16_t>(src_rect.y),
		    static_cast<uint16_t>(src_rect.width), static_cast<uint16_t>(src_rect.height)};
		...
		pixman_image_composite32(PIXMAN_OP_OVER, src.bitmap, mask, bitmap, src_rect.x, src_rect.y,
		                         0, 0, x, y, src_rect.width, src_rect.height);
...
void Bitmap::ToneBlit(int x, int y, Bitmap const& src, Rect const& src_rect, const Tone& tone) {
	...
	if (&src != this)
		pixman_image_composite32(PIXMAN_OP_SRC, src.bitmap, (pixman_image_t*)NULL, bitmap,
		                         src_rect.x, src_rect.y, 0, 0, x, y, src_rect.width,
		                         src_rect.height);

	// FIXME: Saturation looks incorrect (compared to RPG_RT) for values > 128
	if (tone.gray != 128) {
		pixman_color_t gcolor = {static_cast<uint16_t>(tone.gray << 8), 0, 0, 0xFFFF};

		pixman_image_t* gimage = pixman_image_create_solid_fill(&gcolor);

		pixman_image_composite32(PIXMAN_OP_HSL_SATURATION, gimage, src.bitmap, bitmap, src_rect.x,
		                         src_rect.y, 0, 0, x, y, src_rect.width, src_rect.height);
	...
	if (tone.red != 128 || tone.green != 128 || tone.blue != 128) {
		pixman_color_t tcolor = {
		    static_cast<uint16_t>(tone.red << 8),  static_cast<uint16_t>(tone.green << 8),
		    static_cast<uint16_t>(tone.blue << 8), 0xFFFF};

		pixman_image_t* timage = pixman_image_create_solid_fill(&tcolor);

		pixman_image_composite32(PIXMAN_OP_HARD_LIGHT, timage, src.bitmap, bitmap, src_rect.x,
		                         src_rect.y, 0, 0, x, y, src_rect.width, src_rect.height);
...
void Bitmap::BlendBlit(int x, int y, Bitmap const& src, Rect const& src_rect, const Color& color) {
	if (color.alpha == 0) {
		if (&src != this) Blit(x, y, src, src_rect, 255);
		return;
	}
	...
	pixman_image_composite32(PIXMAN_OP_OVER, timage, src.bitmap, bitmap, 0, 0, src_rect.x,
	                         src_rect.y, x, y, src_rect.width, src_rect.height);
...
void Bitmap::FlipBlit(int x, int y, Bitmap const& src, Rect const& src_rect, bool horizontal,
                      bool vertical) {
	...
	pixman_transform_t xform;
	pixman_transform_init_scale(&xform, pixman_int_to_fixed(horizontal ? -1 : 1),
...
void Bitmap::Flip(const Rect& dst_rect, bool horizontal, bool vertical) {
	...
	pixman_image_composite32(PIXMAN_OP_SRC, resampled->bitmap, (pixman_image_t*)NULL, bitmap, 0, 0,
	                         0, 0, dst_rect.x, dst_rect.y, dst_rect.width, dst_rect.height);
...
void Bitmap::Blit2x(Rect const& dst_rect, Bitmap const& src, Rect const& src_rect) {
	pixman_transform_t xform;
	pixman_transform_init_scale(&xform, pixman_double_to_fixed(0.5), pixman_double_to_fixed(0.5));

	pixman_image_set_transform(src.bitmap, &xform);

	pixman_image_composite32(PIXMAN_OP_SRC, src.bitmap, (pixman_image_t*)NULL, bitmap, src_rect.x,
	                         src_rect.y, 0, 0, dst_rect.x, dst_rect.y, dst_rect.width,
	                         dst_rect.height);
...
void Bitmap::GetColorComponents(uint32_t color, uint8_t& r, uint8_t& g, uint8_t& b,
                                uint8_t& a) const {
