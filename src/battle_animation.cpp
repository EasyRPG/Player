...
void BattleAnimationBattle::Draw(Bitmap& dst) {
	if (IsOnlySound())
		return;
	if (animation.scope == lcf::rpg::Animation::Scope_screen) {
		DrawAt(dst, MENU_OFFSET_X + (SCREEN_TARGET_WIDTH / 2), MENU_OFFSET_Y + (SCREEN_TARGET_HEIGHT / 3));
		return;
	}
	...
	for (auto* battler: battlers) {
		...
		DrawAt(dst, MENU_OFFSET_X + battler->GetBattlePosition().x, MENU_OFFSET_Y + battler->GetBattlePosition().y + offset);
	}
}
