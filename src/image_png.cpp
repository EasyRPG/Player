...
static void on_png_warning(png_structp, png_const_charp warn_msg) { Output::Debug("%s", warn_msg); }

static void on_png_error(png_structp, png_const_charp error_msg) { Output::Error("%s", error_msg); }

void ImagePNG::ReadPNG(FILE* stream, const void* buffer, bool transparent, int& width, int& height,
                       void*& pixels) {
	pixels = NULL;

	png_struct* png_ptr =
	    png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, on_png_error, on_png_warning);
	if (png_ptr == NULL) {
