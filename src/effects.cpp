...
void Bitmap::EffectsBlit(int x, int y, Bitmap const& src, Rect const& src_rect, int opacity,
...
// Waver, Split Opacity
void Bitmap::EffectsBlit(int x, int y, Bitmap const& src, Rect const& src_rect, int top_opacity,
                         int bottom_opacity, int opacity_split, int waver_depth,
                         double waver_phase) {
	if (opacity_split <= 0)
		EffectsBlit(x, y, src, src_rect, top_opacity, waver_depth, waver_phase);
	else if (opacity_split >= src_rect.height)
		EffectsBlit(x, y, src, src_rect, bottom_opacity, waver_depth, waver_phase);
	else {
		Rect blit_rect = src_rect;
		blit_rect.height -= opacity_split;
		EffectsBlit(x, y, src, blit_rect, top_opacity, waver_depth, waver_phase);

		blit_rect = src_rect;
		blit_rect.y = src_rect.height - opacity_split;
		blit_rect.height = opacity_split;
		x += src_rect.height - opacity_split;
		EffectsBlit(x, y, src, blit_rect, bottom_opacity, waver_depth, waver_phase);
	}
}
...
void Bitmap::EffectsBlit(int x, int y, Bitmap const& src, Rect const& src_rect, int top_opacity,
	...
	EffectsBlit(x, y, *draw, draw->GetRect(), top_opacity, bottom_opacity,
	            (int)(opacity_split * zoom_y), waver_depth, waver_phase);
}
...
void Bitmap::EffectsBlit(int x, int y, Bitmap const& src, Rect const& src_rect_, int top_opacity,
	...
	bool opacity = (opacity_split <= 0) ? (top_opacity < 255)
	                                    : (opacity_split >= src_rect.height)
	                                          ? (bottom_opacity < 255)
	...
	if (rotate) {
		Matrix fwd = Matrix::Setup(1.0 - angle, zoom_x, zoom_y, (src_rect.width - src_rect.x) / 2,
		                           (src_rect.height - src_rect.y) / 2, x, y);
		EffectsBlit(fwd, src, src_rect, top_opacity, bottom_opacity, opacity_split);
	} else if (scale)
		EffectsBlit(x, y, *draw, src_rect, top_opacity, bottom_opacity, opacity_split, zoom_x,
		            zoom_y, waver_depth, waver_phase);
	else
		EffectsBlit(x, y, *draw, src_rect, top_opacity, bottom_opacity, opacity_split, waver_depth,
		            waver_phase);
}
