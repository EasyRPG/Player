...
std::unique_ptr<lcf::rpg::Save> Game_Interpreter_Shared::ValidateAndLoadSave(const char* caller, const FilesystemView& fs, int save_slot) {
	return ValidateAndLoadSave(caller, fs, save_slot, []() {});
}

std::unique_ptr<lcf::rpg::Save> Game_Interpreter_Shared::ValidateAndLoadSave(const char* caller, const FilesystemView& fs, int save_slot, const std::function<void()> on_corrupted) {
	if (save_slot <= 0) {
