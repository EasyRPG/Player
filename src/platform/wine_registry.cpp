...
namespace {
	...
	struct parse_registry {
		...
		void error(format const& fmt) {
			if (line_number > 0) {
				Output::Warning("Wine registry error: %s\nline %d: \"%s\"", fmt.str().c_str(),
		...
		std::string parse_str() {
			...
			if (not qi::parse(str_it, str_end,
			                  *(escape | ('\\' >> (('x' >> hex) | octal)) | qi::char_),
			                  escaped) or str_it != str_end) {
		...
		section_value parse_value() {
			...
			for (prefix const* pre = prefixes; not pre->pre.empty(); ++pre) {
				if (size_t(i - end) <
				    pre->pre.size() or not std::equal(i, i + pre->pre.size(), pre->pre.begin())) {
					continue;
				}
		...
		void skip_space() {
			while (i < end and std::isspace(*i)) {
				++i;
			}
		}

		parse_registry(std::string const& name)
		    : line_number(0), stream(name.c_str(), std::ios_base::binary | std::ios_base::in) {
			if (not stream) {
				error(format("file open error: \"%s\"") % name);
				return;
			}

			escape.add("\\a", '\a')("\\b", '\b')("\\e", '\x1B')("\\f", '\f')("\\n", '\n')(
			    "\\r", '\r')("\\t", '\t')("\\v", '\v')("\\\\", '\\')("\\\"", '"');
	...
	section_value_opt get_value(section_list const& sec, std::string const& key,
	                            std::string const& val) {
		section_list::const_iterator sec_list_i = sec.find(key);
		if (sec_list_i == sec.end()) {
			return boost::none;
		}
		section::const_iterator sec_i = sec_list_i->second.find(val);
		if (sec_i == sec_list_i->second.end()) {
			return boost::none;
		}
...
std::string Registry::ReadStrValue(HKEY hkey, std::string const& key, std::string const& val) {
	...
	if (ret.size() <
	        3 or not std::isupper(*ret.begin()) or std::string(ret.begin() + 1, ret.begin() + 3) !=
	    ":\\") {
		return ret;
	}
	...
	switch (drive) {
	default:
		path.assign(get_wine_prefix()).append("/drive_").append(&drive, 1).append(ret.begin() + 2,
		                                                                          ret.end());
		break;
	case 'z':
		path.assign(ret.begin() + 2, ret.end());
		break;
	}
