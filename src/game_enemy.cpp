...
Game_Enemy::Game_Enemy(int enemy_id) : Game_Battler() { Setup(enemy_id); }
...
const std::vector<int16_t>& Game_Enemy::GetStates() const { return states; }

std::vector<int16_t>& Game_Enemy::GetStates() { return states; }

const std::string& Game_Enemy::GetName() const { return enemy->name; }

const std::string& Game_Enemy::GetSpriteName() const { return enemy->battler_name; }

int Game_Enemy::GetBaseMaxHp() const { return enemy->max_hp; }

int Game_Enemy::GetBaseMaxSp() const { return enemy->max_sp; }

int Game_Enemy::GetBaseAtk() const { return enemy->attack; }

int Game_Enemy::GetBaseDef() const { return enemy->defense; }

int Game_Enemy::GetBaseSpi() const { return enemy->spirit; }

int Game_Enemy::GetBaseAgi() const { return enemy->agility; }

int Game_Enemy::GetHp() const { return hp; }

int Game_Enemy::GetSp() const { return sp; }
...
int Game_Enemy::GetBattleX() const { return (x * SCREEN_TARGET_WIDTH / 320); }

int Game_Enemy::GetBattleY() const { return (y * SCREEN_TARGET_HEIGHT / 240); }

void Game_Enemy::SetBattleX(int new_x) { x = new_x; }

void Game_Enemy::SetBattleY(int new_y) { y = new_y; }

int Game_Enemy::GetHue() const { return enemy->battler_hue; }

void Game_Enemy::SetHidden(bool _hidden) { hidden = _hidden; }

bool Game_Enemy::IsHidden() const { return hidden; }
...
int Game_Enemy::GetBattleAnimationId() const { return 0; }

Game_Battler::BattlerType Game_Enemy::GetType() const { return Game_Battler::Type_Enemy; }

int Game_Enemy::GetExp() const { return enemy->exp; }

int Game_Enemy::GetMoney() const { return enemy->gold; }
...
bool Game_Enemy::IsActionValid(const RPG::EnemyAction& action) {
	...
	case RPG::EnemyAction::ConditionType_actors: {
		int count = 0;
		/* TODO
		for (std::vector<Battle::Enemy>::const_iterator it = Game_Battle::enemies.begin(); it !=
		Game_Battle::enemies.end(); it++)
