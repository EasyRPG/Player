...
void Window_SaveFile::Refresh() {
	...
	std::ostringstream out;
	out << Data::terms.file << std::setw(2) << std::setfill(' ') << index + 1;
	contents->TextDraw(4, 2, has_save ? Font::ColorDefault : Font::ColorDisabled, Font::Default(), out.str());

	if (corrupted) {
		contents->TextDraw(4, 16 + 2, Font::ColorKnockout, Font::Default(), "Savegame corrupted");
		return;
	}
	...
	contents->TextDraw(8, 16 + 2, Font::ColorDefault, Font::Default(), hero_name);

	contents->TextDraw(8, 32 + 2, 1, Font::Default(), Data::terms.lvl_short);

	int lx = Font::Default()->GetSize(Data::terms.lvl_short).width;
	out.str("");
	out << std::setw(2) << std::setfill(' ') << hero_level;
	contents->TextDraw(8 + lx, 32 + 2, Font::ColorDefault, Font::Default(), out.str());

	contents->TextDraw(42, 32 + 2, 1, Font::Default(), Data::terms.hp_short);

	int hx = Font::Default()->GetSize(Data::terms.hp_short).width;
	out.str("");
	out << hero_hp;
	contents->TextDraw(42 + hx, 32 + 2, Font::ColorDefault, Font::Default(), out.str());
