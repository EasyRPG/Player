...
int Window_Shop::GetChoice() const { return choice; }

void Window_Shop::SetChoice(int nchoice) { choice = nchoice; }
	...
	if (active) {
		...
		case Scene_Shop::BuySellLeave2:
			...
			if (Input::IsTriggered(Input::DECISION)) {
				...
				if (index == buy_index) choice = Scene_Shop::Buy;
				if (index == sell_index) choice = Scene_Shop::Sell;
				if (index == leave_index) choice = Scene_Shop::Leave;
			}
			break;
		}
	}
