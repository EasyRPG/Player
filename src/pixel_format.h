...
struct Component {
	...
	static inline int count_bits(uint32_t mask) {
		int count = 0;
		if ((mask & 0xFFFF0000) != 0) count += 16, mask >>= 16;
		if ((mask & 0xFF00) != 0) count += 8, mask >>= 8;
		if ((mask & 0xF0) != 0) count += 4, mask >>= 4;
		if ((mask & 0xC) != 0) count += 2, mask >>= 2;
		if ((mask & 0x2) != 0) count += 1, mask >>= 1;
		if ((mask & 0x1) != 0) count++;
		return count;
	}
	...
	inline bool operator==(const Component& c) { return mask == c.mask; }
	...
	inline uint32_t pack(const uint8_t& x) const { return (((uint32_t)x >> (8 - bits)) << shift); }
	...
	Component(uint32_t mask) : mask(mask) { convert_mask(); }
};
...
public:
	...
	DynamicFormat(int bits, int rb, int rs, int gb, int gs, int bb, int bs, int ab, int as,
	              PF::AlphaType alpha_type)
	    : bits(bits)
	    , bytes((bits + 7) / 8)
	    , r(rb, rs)
	...
	DynamicFormat(int bits, uint32_t rmask, uint32_t gmask, uint32_t bmask, uint32_t amask,
	              PF::AlphaType alpha_type)
	    : bits(bits)
	    , bytes((bits + 7) / 8)
	    , r(rmask)
	...
	void Set(int _bits, int rb, int rs, int gb, int gs, int bb, int bs, int ab, int as,
	...
	void Set(int _bits, uint32_t rmask, uint32_t gmask, uint32_t bmask, uint32_t amask,
	         PF::AlphaType _alpha_type) {
		bits = _bits;
		bytes = (bits + 7) / 8;
		r = Component(rmask);
	...
	inline int code(bool shifts) const {
		...
		if (!shifts) return x;
		return x | (r.shift << 7) | (g.shift << 12) | (b.shift << 17);
	}
	...
	inline void uint32_to_rgba(uint32_t pix, uint8_t& _r, uint8_t& _g, uint8_t& _b,
	                           uint8_t& _a) const {
		_r = r.unpack(pix);
		_g = g.unpack(pix);
		_b = b.unpack(pix);
	...
	inline uint32_t rgba_to_uint32_t(const uint8_t& _r, const uint8_t& _g, const uint8_t& _b,
	                                 const uint8_t& _a) const {
...
struct bits_traits<TPF, 8> {
	static inline uint32_t get_uint32_t(const uint8_t* p) { return (uint32_t) * (const uint8_t*)p; }
	static inline void set_uint32_t(uint8_t* p, uint32_t pix) { *(uint8_t*)p = (uint8_t)pix; }
...
struct bits_traits<TPF, 16> {
	...
	static inline void set_uint32_t(uint8_t* p, uint32_t pix) { *(uint16_t*)p = (uint16_t)pix; }
	static inline void copy_pixel(uint8_t* dst, const uint8_t* src) {
...
struct bits_traits<TPF, 24> {
	static inline uint32_t get_uint32_t(const uint8_t* p) {
		return ((uint32_t)(p[TPF::endian(2)]) << 16) | ((uint32_t)(p[TPF::endian(1)]) << 8) |
	...
	static inline void copy_pixel(uint8_t* dst, const uint8_t* src) { std::memcpy(dst, src, 3); }
	static inline void set_pixels(uint8_t* dst, const uint8_t* src, int n) {
		for (int i = 0; i < n; i++) copy_pixel(dst + i * 3, src);
	}
};

template <class TPF>
struct bits_traits<TPF, 32> {
	static inline uint32_t get_uint32_t(const uint8_t* p) { return *(const uint32_t*)p; }
	static inline void set_uint32_t(uint8_t* p, uint32_t pix) { *(uint32_t*)p = pix; }
	static inline void copy_pixel(uint8_t* dst, const uint8_t* src) {
...
struct alpha_type_traits<TPF, PF::DynamicAlpha, _alpha_type> {
	static inline PF::AlphaType alpha_type(const TPF* pf) { return pf->format.alpha_type; }
};
...
struct opacity_type_traits<TPF, PF::StaticAlpha, _opacity_type> {
	static inline PF::OpacityType opacity_type(const TPF* /* pf */) { return _opacity_type; }
};

template <class TPF, PF::OpacityType _opacity_type>
struct opacity_type_traits<TPF, PF::DynamicAlpha, _opacity_type> {
	static inline PF::OpacityType opacity_type(const TPF* pf) { return pf->format.opacity_type; }
};

// Alpha traits

// general case
template <class TPF, bool aligned, bool dynamic_alpha, int alpha_type>
struct alpha_traits {
...
struct alpha_traits<TPF, aligned, PF::DynamicAlpha, PF::ColorKey> {
	...
	static inline void set_alpha(const TPF* pf, uint8_t* p, uint8_t alpha) {
		if (pf->has_alpha() && alpha == 0) pf->set_uint32_t(p, pf->colorkey);
	}
};
...
struct alpha_traits<TPF, aligned, PF::StaticAlpha, PF::ColorKey> {
	...
	static inline void set_alpha(const TPF* pf, uint8_t* p, uint8_t alpha) {
		if (alpha == 0) pf->set_uint32_t(p, pf->colorkey);
	}
};
...
struct alpha_traits<TPF, aligned, PF::StaticAlpha, PF::NoAlpha> {
	static inline uint8_t get_alpha(const TPF* /* pf */, const uint8_t* /* p */) { return 255; }
	static inline void set_alpha(const TPF* /* pf */, uint8_t* /* p */, uint8_t /* alpha */) {}
};
...
struct alpha_traits<TPF, PF::IsAligned, dynamic_alpha, PF::Alpha> {
	...
	static inline void set_alpha(const TPF* pf, uint8_t* p, uint8_t alpha) {
		if (pf->has_alpha()) p[pf->a_byte()] = alpha;
	}
};
...
struct rgba_traits<TPF, PF::IsAligned, dynamic_alpha, PF::Alpha> {
	static inline void get_rgba(const TPF* pf, const uint8_t* p, uint8_t& r, uint8_t& g, uint8_t& b,
	                            uint8_t& a) {
		r = p[pf->r_byte()];
		g = p[pf->g_byte()];
		b = p[pf->b_byte()];
	...
	static inline void set_rgba(const TPF* pf, uint8_t* p, const uint8_t& r, const uint8_t& g,
	                            const uint8_t& b, const uint8_t& a) {
		p[pf->r_byte()] = r;
		p[pf->g_byte()] = g;
		p[pf->b_byte()] = b;
		if (pf->has_alpha()) p[pf->a_byte()] = a;
	}
};
...
struct rgba_traits<TPF, PF::NotAligned, PF::StaticAlpha, PF::ColorKey> {
	...
	static inline void set_rgba(const TPF* pf, uint8_t* p, const uint8_t& r, const uint8_t& g,
	                            const uint8_t& b, const uint8_t& a) {
		const uint32_t pix = a == 0 ? pf->colorkey : pf->rgba_to_uint32_t(r, g, b, a);
		pf->set_uint32_t(p, pix);
	}
};
...
struct rgba_traits<TPF, PF::NotAligned, PF::DynamicAlpha, PF::ColorKey> {
	...
	static inline void set_rgba(const TPF* pf, uint8_t* p, const uint8_t& r, const uint8_t& g,
	                            const uint8_t& b, const uint8_t& a) {
		const uint32_t pix =
		    (pf->has_alpha() && a == 0) ? pf->colorkey : pf->rgba_to_uint32_t(r, g, b, a);
		pf->set_uint32_t(p, pix);
	}
};
...
template <bool DYNAMIC_MASKS, bool DYNAMIC_ALPHA, int BITS, int RB, int RS, int GB, int GS, int BB,
          int BS, int AB, int AS, int ALPHA>
...
template <int BITS, int RB, int RS, int GB, int GS, int BB, int BS, int AB, int AS, int ALPHA>
const DynamicFormat dynamic_traits_t<false, false, BITS, RB, RS, GB, GS, BB, BS, AB, AS,
                                     ALPHA>::format(BITS, RB, RS, GB, GS, BB, BS, AB, AS,
                                                    (PF::AlphaType)ALPHA);
...
public:
	...
	void SetColorKey(int _colorkey) { colorkey = _colorkey; }
...
template <int BITS, bool DYNAMIC_MASKS, bool DYNAMIC_ALPHA, int ALPHA, bool ALIGNED, int RB, int RS,
          int GB, int GS, int BB, int BS, int AB, int AS>
class PixelFormatT : public PixelFormat {
public:
	typedef PixelFormatT<BITS, DYNAMIC_MASKS, DYNAMIC_ALPHA, ALPHA, ALIGNED, RB, RS, GB, GS, BB, BS,
	                     AB, AS> my_type;
	...
	static const PF::OpacityType opacity =
	    ALPHA == PF::NoAlpha ? PF::Opaque : (ALPHA == PF::ColorKey || AB == 1) ? PF::Binary
	                                                                           : PF::Variable;
	...
	typedef dynamic_traits_t<DYNAMIC_MASKS, DYNAMIC_ALPHA, BITS, RB, RS, GB, GS, BB, BS, AB, AS,
	                         ALPHA> dynamic_traits_type;
	...
	PixelFormatT()
	    : dynamic_traits(
	          DynamicFormat(BITS, RB, RS, GB, GS, BB, BS, AB, AS, (PF::AlphaType)ALPHA)) {}
	PixelFormatT(const DynamicFormat& format) : dynamic_traits(format) {}

	static inline int endian(int byte) { return Utils::IsBigEndian() ? (bytes - 1 - byte) : byte; }
	...
	inline uint32_t rgba_to_uint32_t(const uint8_t& r, const uint8_t& g, const uint8_t& b,
	                                 const uint8_t& a) const {
		return (((uint32_t)r >> (8 - r_bits())) << r_shift()) |
		       (((uint32_t)g >> (8 - g_bits())) << g_shift()) |
		       (((uint32_t)b >> (8 - b_bits())) << b_shift()) |
		       (((uint32_t)a >> (8 - a_bits())) << a_shift());
	}

	inline const DynamicFormat& format() const { return dynamic_traits.format; }
	...
	inline PF::AlphaType alpha_type() const { return alpha_type_traits_type::alpha_type(this); }

	inline bool opacity_type() const { return opacity_type_traits_type::opacity_type(this); }

	inline bool has_alpha() const { return alpha_type() != PF::NoAlpha; }
	...
	inline void set_rgba(uint8_t* p, const uint8_t& r, const uint8_t& g, const uint8_t& b,
	                     const uint8_t& a) const {
		rgba_traits_type::set_rgba(this, p, r, g, b, a);
	}

	bool Match(const DynamicFormat& ref) const {
		return bits == ref.bits && (dynamic_alpha || alpha_type() == ref.alpha_type) &&
		       (dynamic_masks ||
		        (r_mask() == ref.r.mask && g_mask() == ref.g.mask && b_mask() == ref.b.mask &&
		         (a_mask() == ref.a.mask || alpha_type() != PF::Alpha)));
	}

	int Bits() const { return bits; }

	const DynamicFormat& Format() const { return dynamic_traits.format; }

	void SetFormat(const DynamicFormat& format) { dynamic_traits.set_format(format); }

	bool HasAlpha() const { return has_alpha(); }
};

#ifndef WORDS_BIGENDIAN
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 16, 8, 8, 8,
                     0, 8, 24> format_B8G8R8A8_a;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 0, 8, 8, 8,
                     16, 8, 24> format_R8G8B8A8_a;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 24, 8, 16,
                     8, 8, 8, 0> format_A8B8G8R8_a;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 8, 8, 16, 8,
                     24, 8, 0> format_A8R8G8B8_a;

typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 16, 8, 8,
                     8, 0, 8, 24> format_B8G8R8A8_k;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 0, 8, 8,
                     8, 16, 8, 24> format_R8G8B8A8_k;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 24, 8,
                     16, 8, 8, 8, 0> format_A8B8G8R8_k;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 8, 8, 16,
                     8, 24, 8, 0> format_A8R8G8B8_k;

typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 16, 8, 8,
                     8, 0, 8, 24> format_B8G8R8A8_n;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 0, 8, 8,
                     8, 16, 8, 24> format_R8G8B8A8_n;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 24, 8, 16,
                     8, 8, 8, 0> format_A8B8G8R8_n;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 8, 8, 16,
                     8, 24, 8, 0> format_A8R8G8B8_n;
#else
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 8, 8, 16, 8,
                     24, 8, 0> format_B8G8R8A8_a;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 24, 8, 16,
                     8, 8, 8, 0> format_R8G8B8A8_a;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 0, 8, 8, 8,
                     16, 8, 24> format_A8B8G8R8_a;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 8, 16, 8, 8, 8,
                     0, 8, 24> format_A8R8G8B8_a;

typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 8, 8, 16,
                     8, 24, 8, 0> format_B8G8R8A8_k;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 24, 8,
                     16, 8, 8, 8, 0> format_R8G8B8A8_k;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 0, 8, 8,
                     8, 16, 8, 24> format_A8B8G8R8_k;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 16, 8, 8,
                     8, 0, 8, 24> format_A8R8G8B8_k;

typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 8, 8, 16,
                     8, 24, 8, 0> format_B8G8R8A8_n;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 24, 8, 16,
                     8, 8, 8, 0> format_R8G8B8A8_n;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 0, 8, 8,
                     8, 16, 8, 24> format_A8B8G8R8_n;
typedef PixelFormatT<32, PF::StaticMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 8, 16, 8, 8,
                     8, 0, 8, 24> format_A8R8G8B8_n;
#endif

typedef PixelFormatT<8, PF::StaticMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 8, 0, 8, 0,
                     8, 0, 0, 0> format_L8_k;

typedef PixelFormatT<32, PF::DynamicMasks, PF::StaticAlpha, PF::Alpha, PF::IsAligned, 0, 0, 0, 0, 0,
                     0, 0, 0> format_dynamic_32_a;
typedef PixelFormatT<32, PF::DynamicMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_32_k;
typedef PixelFormatT<32, PF::DynamicMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_32_n;
typedef PixelFormatT<32, PF::DynamicMasks, PF::DynamicAlpha, PF::NoAlpha, PF::IsAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_32_d;

typedef PixelFormatT<24, PF::DynamicMasks, PF::StaticAlpha, PF::ColorKey, PF::IsAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_24_k;
typedef PixelFormatT<24, PF::DynamicMasks, PF::StaticAlpha, PF::NoAlpha, PF::IsAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_24_n;
typedef PixelFormatT<24, PF::DynamicMasks, PF::DynamicAlpha, PF::NoAlpha, PF::IsAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_24_d;

typedef PixelFormatT<16, PF::DynamicMasks, PF::StaticAlpha, PF::Alpha, PF::NotAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_16_a;
typedef PixelFormatT<16, PF::DynamicMasks, PF::StaticAlpha, PF::ColorKey, PF::NotAligned, 0, 0, 0,
                     0, 0, 0, 0, 0> format_dynamic_16_k;
typedef PixelFormatT<16, PF::DynamicMasks, PF::StaticAlpha, PF::NoAlpha, PF::NotAligned, 0, 0, 0, 0,
                     0, 0, 0, 0> format_dynamic_16_n;
typedef PixelFormatT<16, PF::DynamicMasks, PF::DynamicAlpha, PF::NoAlpha, PF::NotAligned, 0, 0, 0,
                     0, 0, 0, 0, 0> format_dynamic_16_d;
