...
namespace {
	...
	void SetupMultiplayer() {
		...
		if (conn_status_window.get() == nullptr || conn_status_window->IsVisible()) {
			conn_status_window = std::make_unique<Window_Base>(0, 240-30, 100, 30);
			conn_status_window->SetContents(Bitmap::Create(100, 30));
			conn_status_window->SetZ(2106632960);
			//conn_status_window->SetZ(12345678);
		}
		Output::Debug("Game_Map::Setup connected={} mapid={}", connected, Game_Map::GetMapId());
		SetConnStatusWindowText("Disconnected");
		//if (connected) emscripten_websocket_close(socket, 1000, "no reason");
		emscripten_websocket_deinitialize(); //kills every socket for this thread
		std::string room_url = server_url + std::to_string(Game_Map::GetMapId());
...
void Game_Map::Setup(std::unique_ptr<lcf::rpg::Map> map_in) {
	...
	//multiplayer setup
	SetupMultiplayer();
	//Main_Data::game_player->SetSpriteGraphic(Main_Data::game_player->GetSpriteName(), 1);
}

void Game_Map::MainPlayerMoved(int dir) {
	Output::Debug("Main player moved {}", dir);
	SendMainPlayerPos();
}
