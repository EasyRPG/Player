...
namespace InelukiKeyPatch {
	bool HandleScriptFile(StringView file_name, bool is_music, AsyncOp& async_op) {
		if (Player::IsPatchKeyPatch() && file_name.ends_with(".script")) {
...
bool Game_Interpreter::CommandManiacGetSaveInfo(lcf::rpg::EventCommand const& com) {
	...
	auto save = ValidateAndLoadSave("ManiacGetSaveInfo", FileFinder::Save(), save_number, [&]() {
