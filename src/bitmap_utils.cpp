...
// Traits types.
template <class BMU, PF::OpacityType SrcAlpha, PF::OpacityType DstAlpha>
struct blend_traits {
	static inline void blend_pixel(BMU* bmu, uint8_t* dst_pixel, const uint8_t* src_pixel,
	                               int opacity) {
		uint8_t sr, sg, sb, sa;
...
struct overlay_traits<BMU, PF::Binary, DstAlpha> {
	static inline void overlay_pixel(BMU* bmu, uint8_t* dst_pixel, const uint8_t* src_pixel) {
		if (bmu->pf_src.get_alpha(src_pixel) != 0) bmu->copy_pixel(dst_pixel, src_pixel);
	}
};
...
struct copy_traits<BMU, false> {
	...
	static inline void copy_pixels(BMU* bmu, uint8_t* dst_pixels, const uint8_t* src_pixels,
	                               int n) {
		for (int i = 0; i < n; i++) {
...
public:
	...
	void SetPixel(uint8_t* dst_pixels, const uint8_t& r, const uint8_t& g, const uint8_t& b,
	              const uint8_t& a);
	...
	void OpacityScaleBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n, int x, int step,
	                      int opacity);
	...
	void OpacityTransformBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int src_pitch, int x0,
	                          int x1, int y, const Rect& src_rect, const Matrix& inv, int opacity);
	void OverlayTransformBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int src_pitch, int x0,
	                          int x1, int y, const Rect& src_rect, const Matrix& inv);
	void CopyTransformBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int src_pitch, int x0,
	                       int x1, int y, const Rect& src_rect, const Matrix& inv);
	void MaskBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n);
	void HSLBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n, int hue, int sat, int lum,
	             int loff);
	...
	static const bool same_format =
	    !PFsrc::dynamic_masks && !PFdst::dynamic_masks &&
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::GetPixel(const uint8_t* src_pixel, uint8_t& r, uint8_t& g,
                                          uint8_t& b, uint8_t& a) {
	pf_src.get_rgba(src_pixel, r, g, b, a);
}

template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::CheckOpacity(const uint8_t* src_pixels, int n, bool& all,
                                              bool& any) {
	for (int x = 0; x < n; x++) {
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::SetPixel(uint8_t* dst_pixel, const uint8_t& r, const uint8_t& g,
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OpacityBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n,
                                             int opacity) {
	for (int i = 0; i < n; i++) {
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OverlayBlit(uint8_t* dst_pixels, const uint8_t* src_pixels,
                                             int n) {
	for (int i = 0; i < n; i++) {
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OpacityScaleBlit(uint8_t* dst_pixels, const uint8_t* src_pixels,
                                                  int n, int x, int step, int opacity) {
	for (int i = 0; i < n; i++) {
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OverlayScaleBlit(uint8_t* dst_pixels, const uint8_t* src_pixels,
                                                  int n, int x, int step) {
	for (int i = 0; i < n; i++) {
		const uint8_t* p = src_pixels + (x >> FRAC_BITS) * pf_src.bytes;
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::CopyScaleBlit(uint8_t* dst_pixels, const uint8_t* src_pixels,
                                               int n, int x, int step) {
	for (int i = 0; i < n; i++) {
		const uint8_t* p = src_pixels + (x >> FRAC_BITS) * pf_src.bytes;
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OpacityTransformBlit(uint8_t* dst_pixels,
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OverlayTransformBlit(uint8_t* dst_pixels,
                                                      const uint8_t* src_pixels, int src_pitch,
                                                      int x0, int x1, int y, const Rect& src_rect,
                                                      const Matrix& inv) {
	const int sx0 = src_rect.x;
...
void BitmapUtilsT<PFsrc, PFdst>::CopyTransformBlit(uint8_t* dst_pixels, const uint8_t* src_pixels,
                                                   int src_pitch, int x0, int x1, int y,
                                                   const Rect& src_rect, const Matrix& inv) {
	const int sx0 = src_rect.x;
	const int sy0 = src_rect.y;
	const int sx1 = src_rect.x + src_rect.width;
	const int sy1 = src_rect.y + src_rect.height;
...
static inline void RGB_to_HSL(const uint8_t& r, const uint8_t& g, const uint8_t& b, int& h, int& s,
                              int& l) {
	enum RGBOrder {
		O_RGB,
		O_RBG,
		...
		O_BRG,
		O_BGR
	} order = (r > g) ? ((r > b) ? ((g < b) ? O_RBG : O_RGB) : O_BRG)
	...
	case O_RGB:
		c = (r - b);
		h = (c == 0) ? 0 : 0x100 * (g - b) / c + 0x000;
		l2 = (r + b);
		break;
	case O_RBG:
		c = (r - g);
		h = (c == 0) ? 0 : 0x100 * (g - b) / c + 0x600;
		l2 = (r + g);
		break;
	case O_GRB:
		c = (g - b);
		h = (c == 0) ? 0 : 0x100 * (b - r) / c + 0x200;
		l2 = (g + b);
		break;
	case O_GBR:
		c = (g - r);
...
static inline void HSL_to_RGB(const int& h, const int& s, const int& l, uint8_t& r, uint8_t& g,
                              uint8_t& b) {
	int l2 = 2 * l;
...
static inline void HSL_adjust(int& h, int& s, int& l, int hue, int sat, int lmul, int loff) {
	h += hue;
	if (h > 0x600) h -= 0x600;
	s = s * sat / 0x100;
	if (s > 0xFF) s = 0xFF;
...
static inline void RGB_adjust_HSL(uint8_t& r, uint8_t& g, uint8_t& b, int hue, int sat, int lmul,
                                  int loff) {
	int h, s, l;
...
void BitmapUtilsT<PFsrc, PFdst>::ToneBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n,
	...
	for (int i = 0; i < n; i++) {
		...
		if (a != 0) {
			...
			r = (uint8_t)std::max(std::min((r - gray) * factor + gray + tone.red + 0.5, 255.0),
			                      0.0);
			g = (uint8_t)std::max(std::min((g - gray) * factor + gray + tone.green + 0.5, 255.0),
			                      0.0);
			b = (uint8_t)std::max(std::min((b - gray) * factor + gray + tone.blue + 0.5, 255.0),
			                      0.0);
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::BlendBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n,
                                           const Color& color) {
	for (int i = 0; i < n; i++) {
		uint8_t r, g, b, a;
		pf_src.get_rgba(src_pixels, r, g, b, a);
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::OpacityChangeBlit(uint8_t* dst_pixels, const uint8_t* src_pixels,
                                                   int n, int opacity) {
	for (int i = 0; i < n; i++) {
...
void BitmapUtilsT<PFsrc, PFdst>::FlipHV(uint8_t*& pixels_first, uint8_t*& pixels_last, int n) {
	for (int i = 0; i < n; i++) {
		if (pixels_first == pixels_last) break;
...
void BitmapUtilsT<PFsrc, PFdst>::FlipH(uint8_t*& pixels_left, uint8_t*& pixels_right, int n) {
	for (int i = 0; i < n; i++) {
		if (pixels_left == pixels_right) continue;
...
template <class PFsrc, class PFdst>
void BitmapUtilsT<PFsrc, PFdst>::FlipV(uint8_t*& pixels_up, uint8_t*& pixels_down, int n,
                                       uint8_t* tmp_buffer) {
...
void BitmapUtils::ScaleBlit(uint8_t* dst_pixels, const uint8_t* src_pixels, int n, int x, int step,
                            int opacity) {
	bool opacity_blit = opacity < 255;
	bool overlay_blit = GetSrcFormat().alpha_type != PF::NoAlpha;
...
BitmapUtils* BitmapUtils::Create(const DynamicFormat& dst_format, const DynamicFormat& src_format,
                                 bool need_source) {
	if (!maps_initialized) {
	...
	if (need_source) {
		...
		if (it != binary_map.end()) return it->second;
		...
		if (it != unary_map.end()) return it->second;
