...
void Window_VarList::DrawItemValue(int index) {
	if (show_switch) {
		if (!Game_Switches.isValidSwitch(first_var + index)) return;
		DrawItem(index, Font::ColorDefault);
		contents->TextDraw(
		    GetWidth() - 16, 16 * index + 2,
		    (!Game_Switches[first_var + index]) ? Font::ColorCritical : Font::ColorDefault,
		    Game_Switches[first_var + index] ? "[ON]" : "[OFF]", Text::AlignRight);
	} else {
		if (!Game_Variables.isValidVar(first_var + index)) return;
		DrawItem(index, Font::ColorDefault);
		std::stringstream ss;
		ss << Game_Variables[first_var + index];
		contents->TextDraw(
		    GetWidth() - 16, 16 * index + 2,
		    (Game_Variables[first_var + index] < 0) ? Font::ColorCritical : Font::ColorDefault,
...
void Window_VarList::UpdateList(int first_value) {
	...
	for (int i = 0; i < 10; i++) {
		ss.str("");
		if ((show_switch && Game_Switches.isValidSwitch(first_var + i)) ||
		    ((!show_switch && Game_Variables.isValidVar(first_var + i))))
			ss << std::setfill('0') << std::setw(4) << (first_value + i) << ": "
			   << (show_switch ? Game_Switches.GetName(first_value + i)
			                   : Game_Variables.GetName(first_value + i));
...
int Window_VarList::GetIndex() { return GetActive() ? index : hidden_index; }