...
bool Game_Actor::IsItemUsable(int item_id) const {
	...
	if (Player::IsRPG2k3() && lcf::Data::system.equipment_setting == RPG::System::EquipmentSetting_class) {
		auto* cls = GetClass();
...
bool Game_Actor::IsSkillUsable(int skill_id) const {
	...
	for (size_t i = 0; i < skill->attribute_effects.size(); ++i) {
		bool required = skill->attribute_effects[i] && lcf::Data::attributes[i].type == RPG::Attribute::Type_physical;
		if (required) {
...
int Game_Actor::SetEquipment(int equip_type, int new_item_id) {
	...
	const RPG::Item* old_item = lcf::ReaderUtil::GetElement(lcf::Data::items, old_item_id);

	const RPG::Item* new_item = lcf::ReaderUtil::GetElement(lcf::Data::items, new_item_id);
...
void Game_Actor::ChangeEquipment(int equip_type, int item_id) {
	...
	const RPG::Item* item = lcf::ReaderUtil::GetElement(lcf::Data::items, GetWeaponId());
	const RPG::Item* item2 = lcf::ReaderUtil::GetElement(lcf::Data::items, GetShieldId());
...
int Game_Actor::CalculateExp(int level) const {
	const RPG::Class* klass = lcf::ReaderUtil::GetElement(lcf::Data::classes, GetData().class_id);
	...
	if (klass) {
		...
		const RPG::Actor& actor = *lcf::ReaderUtil::GetElement(lcf::Data::actors, actor_id);
		base = actor.exp_base;
...
int Game_Actor::GetStateProbability(int state_id) const {
	...
	for (const auto equipment : GetWholeEquipment()) {
		RPG::Item* item = lcf::ReaderUtil::GetElement(lcf::Data::items, equipment);
		if (item != nullptr
...
int Game_Actor::GetAttributeModifier(int attribute_id) const {
	...
	for (auto id_object : GetWholeEquipment()) {
		RPG::Item *object = lcf::ReaderUtil::GetElement(lcf::Data::items, id_object);
...
int Game_Actor::GetBattleX() const {
	...
	if (GetActor().battle_x == 0 ||
		lcf::Data::battlecommands.placement == RPG::BattleCommands::Placement_automatic) {
		...
		const RPG::Terrain* terrain = lcf::ReaderUtil::GetElement(lcf::Data::terrains, Game_Battle::GetTerrainId());
		if (terrain) {
...
void Game_Actor::ChangeBattleCommands(bool add, int id) {
	...
	if (add) {
		const RPG::BattleCommand* cmd = lcf::ReaderUtil::GetElement(lcf::Data::battlecommands.commands, id);
		if (!cmd) {
...
const std::vector<const RPG::BattleCommand*> Game_Actor::GetBattleCommands() const {
	...
	for (int command_index : obc) {
		...
		const RPG::BattleCommand* cmd = lcf::ReaderUtil::GetElement(lcf::Data::battlecommands.commands, command_index);
		if (!cmd) {
...
int Game_Actor::GetBattleAnimationId() const {
	...
	if (GetData().battler_animation <= 0) {
		...
		if ((GetData().class_id > 0) && GetClass()) {
			...
			const RPG::BattlerAnimation* anima = lcf::ReaderUtil::GetElement(lcf::Data::battleranimations, GetActor().battler_animation);
			if (!anima) {
...
void Game_Actor::RemoveInvalidData() {
	...
	*/
	...
	for (size_t i = 0; i < equipment.size(); ++i) {
		...
		RPG::Item* item = lcf::ReaderUtil::GetElement(lcf::Data::items, eq_id);
	...
	if (GetData().class_id > 0) {
		const RPG::Class* cls = lcf::ReaderUtil::GetElement(lcf::Data::classes, GetData().class_id);
		if (!cls) {
