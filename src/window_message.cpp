...
void Window_Message::FinishMessageProcessing() {
	...
	DebugLog("%d: MSG START CLOSE %d", close_frames);
...
void Window_Message::Update() {
	...
	if (wait_count > 0) {
		DebugLog("%d: MSG WAIT %d", wait_count);
		--wait_count;
		return;
	}

	if (GetPause()) {
		DebugLog("%d: MSG PAUSE");
		WaitForInput();
...
void Window_Message::UpdateMessage() {
	...
	while (true) {
		...
		if (wait_count > 0) {
			DebugLog("%d: MSG WAIT LOOP %d", wait_count);
			--wait_count;
			break;
		}
		...
		if (tret.is_escape && ch != Player::escape_char) {
			...
			case '_':
				...
				DebugLogText("%d: MSG HalfWait \\_");
				SetWaitForCharacter(1);
				break;
			case '$':
				// Show Gold Window
				ShowGoldWindow();
				DebugLogText("%d: MSG Gold \\$");
			...
			case '<':
				...
				DebugLogText("%d: MSG Instant Speed Stop \\<");
				SetWaitForNonPrintable(speed);
				break;
			case '.':
				// 1/4 second sleep
...
bool Window_Message::DrawGlyph(Font& font, const Bitmap& system, char32_t glyph, bool is_exfont) {
	if (is_exfont) {
		...
		if (glyph < 128) {
			DebugLogText("%d: MSG DrawGlyph ASCII %c", static_cast<char>(glyph));
		} else {
			DebugLogText("%d: MSG DrawGlyph UTF32 0x%x", static_cast<int>(glyph));
		}
	}
...
void Window_Message::SetWait(int frames) {
	...
	DebugLogText("%d: MSG SetWait %d", frames);
	wait_count = frames;
}
